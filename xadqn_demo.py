...
default_options = {
    ...
    # IMPORTANT: if set to True it allows lifelong learning with decent bootstrapping
    ...
    "gamma": 0.999,
    # We use an higher gamma to extend the MDP's horizon; optimal agency on GraphDelivery requires a longer horizon.
    ...
    # For some clustering schemes (e.g. extrinsic_reward, moving_best_extrinsic_reward, etc..) it has to be equal to 'complete_episodes', otherwise it can also be 'truncate_episodes'.
    ...
    # Number of 'n_step' transitions per train-batch. Default is: 100 for TD3, 256 for SAC and DDPG, 32 for SAC, 500 for APPO.
    ###########################
...
xa_default_options = {
    ...
    "buffer_options": {
        ...
        # Whether to replay batches with the highest priority/importance/relevance for the agent.
        "centralised_buffer": True,  # for MARL
        'global_size': EXPERIENCE_BUFFER_SIZE,
        # Maximum number of batches stored in the experience buffer. Every batch has size 'rollout_fragment_length' (default is 50).
        ...
        #################
        'cluster_size': None,
        # Default None, implying being equal to global_size. Maximum number of batches stored in a cluster (whose number depends on the clustering scheme) of the experience buffer. Every batch has size 'sample_batch_size' (default is 1).
        ...
        # Whether to use cluster-level information to compute importance weights rather than the whole buffer.
        'clustering_xi': 2,
        #################
        'prioritized_drop_probability': 1,
        # Probability of dropping the batch having the lowest priority in the buffer instead of the one having the lowest timestamp. In SAC default is 0.
