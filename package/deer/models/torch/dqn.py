    ...
    def init(preprocessing_model):
        class TorchAdaptiveMultiHeadDQNInner(DQNTorchModel):
            ...
            def __init__(self, obs_space, action_space, num_outputs,
                         model_config, name, q_hiddens=(256,), dueling=
                         False, dueling_activation="relu", num_atoms=1,
                         use_noisy=False, v_min=-10.0, v_max=10.0,
                         sigma0=0.5, add_layer_norm=False, **kwargs):
                preprocessed_input_size = preprocessing_model(
                    obs_space, model_config['custom_model_config']).get_num_outputs()
                self.add_nonstationarity_correction = model_config[
                    'custom_model_config'].get(
                ...
                self.preprocessing_model = preprocessing_model(
                    obs_space, model_config['custom_model_config'])
            ...
            def forward(self, input_dict, state, seq_lens):
                ...
                if self.add_nonstationarity_correction:
                    if "policy_signature" not in input_dict:
                        ...
                        input_dict["policy_signature"] = torch.from_numpy(
                            np.zeros(
                                (input_dict.count, self.policy_signature_size),
                                dtype=np.float32))
                    model_out = torch.concat(
                        (model_out, input_dict['policy_signature']), dim=-1)
                return model_out, state

            def variables(self, as_dict=False):
                if not as_dict:
                    return self.preprocessing_model.variables(
                        as_dict) + super().variables(as_dict)
