...
def get_input_recursively(_obs_space, valid_key_fn=lambda x: True):
    if isinstance(_obs_space, gym.spaces.Dict):
        space_iter = (v for k, v in _obs_space.spaces.items() if
                      valid_key_fn(k))
        return list(itertools.chain.from_iterable(
            map(get_input_recursively, space_iter)))
    elif isinstance(_obs_space, gym.spaces.Tuple):
        return list(itertools.chain.from_iterable(
            map(get_input_recursively, _obs_space.spaces)))
        ...
        return list(itertools.chain.from_iterable(
            map(get_input_recursively, _obs_space)))
    return [_obs_space]
