...
def get_checkpoint_n_logger_by_experiment_id(
        trainer_class, environment_class,
        experiment_id, results_dir=DEFAULT_RESULTS_DIR):
    if experiment_id is None:
        return 0, None, None
    # timestr = datetime.today().strftime("%Y-%m-%d_%H-%M-%S")
    logdir_prefix = "{}_{}_{}".format(
        trainer_class.__name__, environment_class, experiment_id)
    logdir = os.path.join(results_dir, logdir_prefix)
    if not os.path.exists(logdir):
        os.makedirs(logdir)
    logger_creator_fn = lambda config: UnifiedLogger(config, logdir,
                                                     loggers=None)
    checkpoint_n, checkpoint = find_last_checkpoint_in_directory(logdir)
    return checkpoint_n, checkpoint, logger_creator_fn


def restore_agent_from_checkpoint(alg_class, config, environment_class,
                                  checkpoint):
...
def test(agent, config, environment_class, checkpoint_directory,
         save_gif=True, delete_screens_after_making_gif=True,
         compress_gif=True, n_episodes=3, with_log=False):
    """Tests and renders a previously trained model"""
    # agent = restore_agent_from_checkpoint(
    #     tester_class, config, environment_class, checkpoint)

    env = agent.env_creator(config["env_config"])
    # Atari wrapper
    if is_atari(env) and not config.get("custom_preprocessor") and config.get(
    ...
    def print_screen(screens_dir, frame_step):
        filename = os.path.join(screens_dir, f'frame{frame_step}.jpg')
        if 'rgb_array' in render_modes:
            ...
            raise Exception(
                f"No compatible render mode (rgb_array,ansi,ascii,human) in "
                f"{render_modes}.")
        return filename

    screens_directory = None
    log_list, file_list = None, None
    multiagent = isinstance(env, MultiAgentEnv)
    stats_dict_list = []
    step_list = []
    for episode_id in tqdm(range(n_episodes), total=n_episodes):
        if with_log or save_gif:
            episode_directory = os.path.join(checkpoint_directory,
        ...
        if multiagent:
            ...
            if screens_directory is not None:
                file_list = [print_screen(screens_directory, step)]
            while not done_dict['__all__'] and (
                    not config.get('horizon', None) or
                    step < config['horizon']):
                step += 1
                # action = env.action_space.sample()
                action_dict = {
                    k: agent.get_policy(
                        policy_mapping_fn(k)).compute_single_action(
                        v, explore=False)[0]
                    for k, v in state_dict.items()
                    if not done_dict.get(k, True)
                }
                # print('action_dict', action_dict)
                state_dict, reward_dict, done_dict, info_dict = env.step(
                    action_dict)
                # state_dict = dict(zip(state_dict.keys(),map(np.squeeze,state_dict.values())))
                stats_dict['reward'] += sum(reward_dict.values())
                if done_dict['__all__']:
                    stats_dict.update(
                        list(info_dict.values())[-1]["stats_dict"])
            ...
            if save_gif:
                file_list = [print_screen(screens_directory, step)]
            while not done and (
                    not config.get('horizon', None) or step < config[
                'horizon']):
                step += 1
                # action = env.action_space.sample()
        ...
        if with_log:
            with open(os.path.join(episode_directory,
                                   f'episode_{step}_{sum_reward}.log'),
                      'w') as f:
        ...
        if save_gif:
            ...
            if compress_gif:
                with zipfile.ZipFile(gif_file_path + '.zip', mode='w',
...
def train(trainer_class, config_class, config_dict, environment_class,
          experiment=None, test_every_n_step=None,
          stop_training_after_n_step=None, log=True, save_gif=True,
          delete_screens_after_making_gif=True, compress_gif=True,
          n_episodes=3, with_log=False):
    # os.environ["OMP_NUM_THREADS"] = 1
    _, checkpoint, logger_creator_fn = get_checkpoint_n_logger_by_experiment_id(
        trainer_class, environment_class, experiment)
    ...
    agent = trainer_class(config, env=environment_class,
                          logger_creator=logger_creator_fn)
    if checkpoint:
    ...
    check_steps = test_every_n_step if test_every_n_step is not None \
        else float('inf')
    ...
    def save_checkpoint():
        ...
        print(f'Testing..')
        try:
            test(agent, config.to_dict(), environment_class, checkpoint,
                 save_gif=save_gif,
                 delete_screens_after_making_gif=delete_screens_after_making_gif,
                 compress_gif=compress_gif, n_episodes=n_episodes,
                 with_log=with_log)
