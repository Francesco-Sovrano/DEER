...
from imageio import get_writer as imageio_get_writer, \
    imread as imageio_imread  # GIFs
...
def line_plot(logs, figure_file, max_plot_size=20, max_length=None,
              show_deviation=False, base_list=None,
              base_shared_name='baseline', average_non_baselines=None,
              buckets_average='median'):
    assert not base_list or len(base_list) == len(
        logs), (f"base_list (len {len(base_list)}) "
                f"and logs (len {len(logs)}) must have same lenght or base_"
    ...
    axes = [figure.add_subplot(grid[id // ncols, id % ncols]) for id in
            range(max_stats_count)]
    # Populate axes
    lines_dict = {}
        ...
        for _ in range(plot_size):
            ...
            for key in stat:  # foreach statistic
                ...
                stats_dict["quantiles"].append({
                    'lower_quartile': float(np.quantile(value_list, 0.25)),
                    # lower quartile
                    'median': float(np.quantile(value_list, 0.5)),  # median
                    'upper_quartile': float(np.quantile(value_list, 0.75)),
                    # upper quartile
                })
                ...
                stats_dict["min"] = float(
                    min(stats_dict["min"], min(value_list)))
                stats_dict["max"] = float(
                    max(stats_dict["max"], max(value_list)))
    ...
    for name, line in lines_dict.items():
        ...
        for j in range(ncols):
            for i in range(nrows):
                ...
                y_key_lower_quartile, y_key_median, y_key_upper_quartile = map(
                    np.array, zip(*unpack_quantiles(y_key["stats_to_plot"])))
                if base_list:
                    base_line = base_list[log_id]
                    base_y_key = lines_dict[base_line]['y'][key]
                    base_y_key_lower_quartile, base_y_key_median, base_y_key_upper_quartile = map(
                        np.array,
                        zip(*unpack_quantiles(base_y_key["stats_to_plot"])))
                    normalise = lambda x, y: 100 * (x - y) / (
                                y - base_y_key['min'] + 1)
                    y_key_median = normalise(y_key_median, base_y_key_median)
                    y_key_lower_quartile = normalise(y_key_lower_quartile,
                                                     base_y_key_lower_quartile)
                    y_key_upper_quartile = normalise(y_key_upper_quartile,
                                                     base_y_key_upper_quartile)
                # print stats
    ...
    for log_id, (name, plot_list) in enumerate(plot_dict.items()):
        for plot in plot_list:
            ...
            ax.set_ylabel(wrap_string(format_label(
                key) if not base_list else f'{format_label(key)} - % of gain over baseline',
                                      25), fontdict=font_dict)
            ax.set_xlabel('step', fontdict=font_dict)
            # plot mean line
            ax.plot(x_key, y_key_median, label=format_label(name),
                    linestyle=linestyle_set[log_id // len(color_set)],
                    color=color_set[log_id % len(color_set)])
            # plot std range
            if show_deviation:
                ax.fill_between(x_key, y_key_lower_quartile,
...
def list_progress_csv_files(results_dir):
    path = Path(results_dir)
    assert path.is_dir()
    contents = path.iterdir()
    subdirectories = [entry for entry in contents if entry.is_dir()]
    ...
    for subdir in subdirectories:
        ...
        runs_in_subdir = [run for run in subdir.iterdir()
                          if run.is_dir()]
        progress_csv_files = [file for run in runs_in_subdir for file in
                              run.iterdir() if
        ...
        print(f"experiment {exp_name}: {progress_csv_file}")
    return all_csv_files, all_names


def line_plot_files(url_list, name_list, figure_file, max_step=None,
                    max_plot_size=20, show_deviation=False, base_list=None,
                    base_shared_name='baseline', average_non_baselines=None,
                    statistics_list=None, buckets_average='median',
                    step_type='num_env_steps_sampled'):
    assert len(url_list) == len(name_list), \
        (f"url_list (len {len(url_list)}) and name_list (len "
         f"{len(name_list)}) must have same lenght")

    if Path(url_list[0]).is_dir():
        url_list, name_list = list_progress_csv_files(url_list[0])
    ...
    for url, name in zip(url_list, name_list):
        ...
        logs.append({
            'name': name,
            'data_iter': parse(
                url, max_step=max_step,
                statistics_list=statistics_list, step_type=step_type),
            'length': length,
            'line_example': parse_line(
                line_example, statistics_list=statistics_list,
                step_type=step_type)
        })
    max_length = max(logs, key=lambda x: x['length'])['length']
    line_plot(logs, figure_file, max_plot_size=max_plot_size,
              max_length=max_length, show_deviation=show_deviation,
              base_list=base_list, base_shared_name=base_shared_name,
              average_non_baselines=average_non_baselines,
              buckets_average=buckets_average)


def parse_line(line, statistics_list=None, step_type='num_env_steps_sampled'):
    key_list = line.columns.tolist()
    get_keys = lambda k: list(map(lambda x: x[len(k):].strip('/'),
                                  filter(lambda x: x.startswith(k), key_list)))
    get_element = lambda df, key: df[key].tolist()[0] if key in df else None
    arrayfy = lambda x: np.array(
        x[1:-1].split(', ') if ', ' in x[1:-1] else x[1:-1].split(' '),
        dtype=np.float32) if x[1:-1] else []

    step = get_element(line,
                       f"info/{step_type}")  # "num_env_steps_sampled", "num_env_steps_trained", "num_agent_steps_sampled", "num_agent_steps_trained"
    # obj = {
    ...
    obj = {
        "episode_reward_median": np.median(arrayfy(
            get_element(line, "hist_stats/episode_reward").replace('$', ' '))),
    }
    for k in ["episode_reward_mean", "episode_reward_max",
              "episode_reward_min", "episode_len_mean", "episodes_total"]:
        obj[k] = get_element(line, k)

    agent_names = get_keys('policy_reward_mean/')
    if agent_names:
        for agent_id in agent_names:
            for k in ["policy_reward_mean", "policy_reward_max",
                      "policy_reward_min"]:
                obj[f'{agent_id}_{k}'] = get_element(line, f"{k}/{agent_id}")
            obj[f"{agent_id}_policy_reward_median"] = np.median(np.median(
                arrayfy(get_element(line,
                                    f"hist_stats/policy_{agent_id}_reward").replace(
                    '$', ' ')), axis=-1))
    else:
        agent_names = ["default_policy"]

    get_label = (lambda i, x: f"{i}_{x}") if len(agent_names) > 1 else (
        lambda i, x: x)
    for agent_id in agent_names:
        obj.update({
            f"info/learner/{agent_id}/{k}": get_element(line,
                                                        f"info/learner/{agent_id}/{k}")
            for k in get_keys(f"info/learner/{agent_id}/")
            if isinstance(get_element(line, f"info/learner/{agent_id}/{k}"),
                          numbers.Number)
        })
        obj.update({
            f"buffer/{agent_id}/{k}": get_element(line,
                                                  f"buffer/{agent_id}/{k}")
            for k in get_keys(f"buffer/{agent_id}/")
            if isinstance(get_element(line, f"buffer/{agent_id}/{k}"),
                          numbers.Number)
        })
    obj.update({
...
def ascii_image(string, file_name):
    ...
    for line in splitlines:
        text_size = font.getsize(
            line)  # for efficiency's sake, split only on the first newline, discard the rest
