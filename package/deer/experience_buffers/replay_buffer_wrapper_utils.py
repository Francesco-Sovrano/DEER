...
from deer.experience_buffers.buffer.pseudo_prioritized_buffer import \
    PseudoPrioritizedBuffer, get_batch_infos, get_batch_indexes, get_batch_uid, \
    discard_batch
from deer.experience_buffers.buffer.hierarchical_prioritized_buffer import \
    HierarchicalPrioritizedBuffer
...
def get_clustered_replay_buffer(config, siamese=False):
    assert config.batch_mode == "complete_episodes" or not \
    config.clustering_options[
        "cluster_with_episode_type"], f"This algorithm requires 'complete_episodes' as batch_mode when 'cluster_with_episode_type' is True"
    clustering_scheme_type = config.clustering_options.get("clustering_scheme",
                                                           None)
    # no need for unclustered_buffer if clustering_scheme_type is none
    ratio_of_samples_from_unclustered_buffer = config.clustering_options[
        "ratio_of_samples_from_unclustered_buffer"] if clustering_scheme_type else 0
    local_replay_buffer = LocalReplayBuffer(
        config.buffer_options,
        # learning_starts=config.num_steps_sampled_before_learning_starts,
        seed=config.seed,
        cluster_selection_policy=config.clustering_options[
            "cluster_selection_policy"],
        ratio_of_samples_from_unclustered_buffer=ratio_of_samples_from_unclustered_buffer,
        siamese=siamese,
    )
    clustering_scheme = ClusterManager(clustering_scheme_type,
                                       config.clustering_options[
...
def assign_types(multi_batch, clustering_scheme, batch_fragment_length,
    ...
    if not isinstance(multi_batch, MultiAgentBatch):
        multi_batch = MultiAgentBatch({DEFAULT_POLICY_ID: multi_batch},
                                      multi_batch.count)
    ...
    for pid, meta_batch in multi_batch.policy_batches.items():
        batch_dict[pid] = []
        batch_list = meta_batch.split_by_episode() if with_episode_type else [
            meta_batch]
        for batch in batch_list:
            sub_batch_count = int(np.ceil(len(batch) / batch_fragment_length))
            sub_batch_list = [
                batch[
                i * batch_fragment_length: (i + 1) * batch_fragment_length]
                for i in range(sub_batch_count)
            ] if len(batch) > batch_fragment_length else [batch]
            episode_type = clustering_scheme.get_episode_type(
                sub_batch_list) if with_episode_type else None
            for i, sub_batch in enumerate(sub_batch_list):
                ...
                sub_batch[SampleBatch.INFOS] = [{'batch_type': batch_type,
                                                 'training_step': training_step}]  # remove unnecessary infos to save some memory
...
def apply_to_batch_once(fn, batch_list):
    ...
    return list(
        map(lambda x: updated_batch_dict[get_batch_uid(x)], batch_list))
...
class MultiAgentBatchWithDefaultAgent(MultiAgentBatch):
    ...
    def from_multi_agent_batch(b, agent_id=None):
        ...
        return MultiAgentBatchWithDefaultAgent(b.policy_batches, b.count,
                                               agent_id)
...
class LocalReplayBuffer(ParallelIteratorWorker):
    ...
    def __init__(self,
        ...
        logger.warning(
            f'Building LocalReplayBuffer with centralised_buffer = {self.centralised_buffer}')
        self.replay_integral_multi_agent_batches = self.buffer_options.get(
        ...
        self.batch_dropout_rate = self.buffer_options.get('batch_dropout_rate',
                                                          0)
        ...
        def new_buffer():
            return self.buffer_class(**self.buffer_options, seed=seed) if (
                self.prioritized_replay) else Buffer(**self.buffer_options,
                                                     seed=seed)

        self.replay_buffers = collections.defaultdict(new_buffer)
        self.ratio_of_old_elements = np.clip(
            1 - ratio_of_samples_from_unclustered_buffer, 0, 1)
        self.buffer_of_recent_elements = collections.defaultdict(
            new_buffer) if ratio_of_samples_from_unclustered_buffer > 0 else None
    ...
    def add_batch(self, batch, update_prioritisation_weights=False):
        ...
        with self.add_batch_timer:
            ...
            for policy_id in batch.policy_batches.keys():
                sub_batch = MultiAgentBatchWithDefaultAgent.from_multi_agent_batch(
                    batch, policy_id)
                ...
                if not isinstance(batch_type, (tuple, list)):
                    ...
                    sub_type_list = tuple(filter(
                        lambda x: not self.replay_buffers[
                            buffer_id].is_valid_cluster(x), batch_type))
                    ...
                    cluster_cumsum = np.cumsum(list(
                        map(lambda x: self.replay_buffers[
                                          buffer_id].get_cluster_size(x) + 1,
                            batch_type)))
                    cluster_mass = random.random() * cluster_cumsum[-1]  # O(1)
                    batch_type_idx, _ = next(
                        filter(lambda x: x[-1] >= cluster_mass, enumerate(
                            cluster_cumsum)))  # O(|self.type_keys|)
                    sub_type_list = (batch_type[batch_type_idx],)
                elif self._cluster_selection_policy == 'max':
                    sub_type_list = (max(
                        batch_type,
                        key=lambda x: (
                        self.replay_buffers[buffer_id].get_cluster_size(x),
                        random.random())
                    ),)
                ...
                for sub_type in sub_type_list:
                    ...
                    sub_batch = MultiAgentBatchWithDefaultAgent.from_multi_agent_batch(
                        batch.copy(), policy_id)
                    # Make a deep copy of infos so that for every sub_type the infos dictionary is different
                    sub_batch[SampleBatch.INFOS] = copy.deepcopy(
                        sub_batch[SampleBatch.INFOS])
                    self.replay_buffers[buffer_id].add(batch=sub_batch,
                                                       type_id=sub_type,
                    ...
                    if self.buffer_of_recent_elements is not None:
                        ...
                        sub_batch[SampleBatch.INFOS] = copy.deepcopy(
                            sub_batch[SampleBatch.INFOS])
                        self.buffer_of_recent_elements[buffer_id].add(
                            batch=sub_batch,
                            update_prioritisation_weights=update_prioritisation_weights)
    ...
    def replay(self, batch_count=1, cluster_overview_size=None,
               update_replayed_fn=None):
        output_batches = []
        if self.buffer_of_recent_elements is not None:
            n_of_old_elements = max(1, int(np.ceil(
                batch_count * self.ratio_of_old_elements)))  # random.randint(0,batch_count)
    ...
    def sample_from_buffer(self, buffer_dict, batch_count=1,
        ...
        with self.replay_timer:
            ...
            buffer_dict_items = [x for x in buffer_dict.items() if
                                 not x[-1].is_empty()]
            for buffer_idx, (policy_id, replay_buffer) in enumerate(
                    buffer_dict_items):
                # if replay_buffer.is_empty():
                # 	continue
                # batch_iter = replay_buffer.sample(batch_count)
                batch_size_list = [cluster_overview_size] * (
                ...
                if update_replayed_fn:
                    self._buffer_lock.acquire_write()
                    batch_iter = apply_to_batch_once(update_replayed_fn,
                                                     batch_iter)
                    self._buffer_lock.release_write()
                if not self.replay_integral_multi_agent_batches:
                    ...
                    for i, batch in enumerate(batch_iter):
                        ...
                        if (buffer_idx + i) % len(
                                buffer_dict_items) == 0:  # every batch has information about every agent, hence we take batch_list/len(buffer_dict_items) batches per buffer with this formula
                            batch_list[i] = batch.to_multi_agent_batch()
        # print(batch_list)
        return (
            MultiAgentBatch(
                samples, max(map(lambda x: x.count, samples.values())))
    ...
    def get_train_steps(self):
        return max((replay_buffer.timesteps for replay_buffer in
                    self.replay_buffers.values())) if self.replay_buffers else 0
    ...
    def update_priorities(self, prio_dict):
        ...
        with self.update_priorities_timer:
            ...
            for policy_id, new_batch in prio_dict.items():
                for type_id, batch_index in get_batch_indexes(
                        new_batch).items():
                    self.replay_buffers[policy_id].update_priority(new_batch,
                                                                   batch_index,
                                                                   type_id)
                    if self.buffer_of_recent_elements is not None:
                        self.buffer_of_recent_elements[
                            policy_id].update_priority(new_batch, batch_index)
            self._buffer_lock.release_write()
